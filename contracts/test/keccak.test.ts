...
describe("Merkle Tree Test", function () {
  ...
  it("merkle tree changes should track", async () => {
    ...
    const hasherFn = (input: string) => {
      ...
      const concatenated = new Uint8Array([...left, ...right]);

      const hash = keccak256(concatenated).toString();
      return hash;
    };
    ...
    expect(await PUBTCK.isKnownRoot("0x" + root)).to.be.true;

    const alicePrivateKey =
      BigInt(alice.privateKey) %
      21888242871839275222246405745257275088548364400416034343698204186575808495617n;
    const alicePosAddress = keccak256(
      new Uint8Array([...new TextEncoder().encode(alicePrivateKey.toString())]),
    );

    const bobPrivateKey =
      BigInt(bob.privateKey) %
      21888242871839275222246405745257275088548364400416034343698204186575808495617n;
    const bobPosAddress = keccak256(
      new Uint8Array([...new TextEncoder().encode(bobPrivateKey.toString())]),
    );

    const charliePrivateKey =
      BigInt(charlie.privateKey) %
      21888242871839275222246405745257275088548364400416034343698204186575808495617n;
    const charliePosAddress = keccak256(
      new Uint8Array([
        ...new TextEncoder().encode(charliePrivateKey.toString()),
      ]),
    );
